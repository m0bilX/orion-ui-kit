...
type TableProps = {
    ...
    rowRenderer: (index: number, data: any, isOpen?: boolean, onClick?: (index: number) => void) => JSX.Element;
    data: any[];
...
export const Table: FC<TableProps> = (props) => {
    ...
    const headerCols = props.headers.map((h, i) => {
        return <div key={`hcol${i}`} className={`${h.className} ${getSortClass(i)}`} onClick={() => onHeaderClick(i)}>
            {h.text}
        </div>
    });
    ...
    for (let i = 0; i < sortedData.length; i++) {
        rows.push(props.rowRenderer(i, sortedData[i], i === openIndex, (index: number) => setOpenIndex(openIndex === index ? -1 : index)));
    }
    ...
    return (
        <div className={props.className}>
            ...
            <div className={props.scrollContainerClassName}>
                {rows}
            </div>
